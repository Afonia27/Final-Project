...
/*
 Created by Afanasi Chihaioglo. 25/01/19
Program to send shadow update request to IoT thing
Created with the help of the open source Linkit One examples
 */
...
//typedef struct {     // AF: Structures which does not have any use in the program. Most probably, the developer wanted to make program as object oriented as possible
//  double temperature;
//  bool windowOpen;
//} ShadowReported;
//
...
float voltage = 0.0; // AF: Initiate the variables
float current = 0.0;
float positio = 0.0; 
...
jsonStruct_t voltageHandler;  // AF: Voltage placeholder
jsonStruct_t currentHandler; // AF: Current placeholder
jsonStruct_t statusHandler; 
...
{      
	      ...
	      aws_iot_mqtt_init(&mqttClient);
              ...
              //windowActuator.cb = windowActuate_Callback; // Commented out since do not need
              //windowActuator.pData = &windowOpen;
              //windowActuator.pKey = "windowOpen"; 
              //windowActuator.type = SHADOW_JSON_BOOL;

              // IF YOU WOULD LIKE TO ADD MORE DATA ADD IT HERE AND ATTACH IT ON THE LINE 415
              voltageHandler.cb = NULL;              // Initiate data structure for voltage, current, temperature
              voltageHandler.pKey = "voltage";
              voltageHandler.pData = &voltage;
              voltageHandler.type = SHADOW_JSON_FLOAT;
              ...
              statusHandler.pKey = "status";
              statusHandler.pData = &positio;
              statusHandler.type = SHADOW_JSON_FLOAT;
...
int publish_Shadow(char * topic, char * message) {
    ...
    if (rc == NONE_ERROR) {
      //Important setting to change if you would like to extend data structure 
        rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 4, &voltageHandler, &currentHandler, &temperatureHandler, &statusHandler ); // Specify reported Handlers, 3 represents number of handlers
      // 
        if (rc == NONE_ERROR) {
